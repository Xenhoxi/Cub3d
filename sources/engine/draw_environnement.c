...
/*   Updated: 2023/10/13 16:11:35 by ljerinec         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	drawline(t_cub *cub, t_line *line, int draw_start, int draw_end)
{
	...
	line->img_text = mlx_new_image(cub->mlx, 5, draw_end - draw_start);
	if (!line->img_text)
		return ;
	...
	mlx_image_to_window(cub->mlx, line->img_text, line->i * 5, WIN_HEIGHT / 2 - WIN_HEIGHT / 4);
}
...
void	draw_vision(t_cub *cub, t_line *line, int fdist)
{
	...
	line_height = (int)(H / line->reel_dist * 10);
	draw_start = -line_height / 2 + H / 4;
	if (draw_start < 0)
		draw_start = 0;
	draw_end = line_height / 2 + H / 4;
	if (draw_end >= H)
		draw_end = H - 1;
	// printf("----------Ray------------\n");
